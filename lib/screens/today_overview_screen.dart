...
class TodayOverviewScreen extends StatelessWidget {
  /// 当前时间，默认为系统时间
  /// 通过构造函数传入可以避免build方法中直接调用DateTime.now()导致的性能问题
  ...
  Widget build(BuildContext context) {
    ...
    final todayMedicals = medicalProvider.records
        ...
        .toList();
    // 未完成备忘
    final pendingMemos = memoProvider.memos
              ...
              children: [
                ...
                Expanded(
                  child: _StatCard(
                    ...
                    subText: todayMedicals.isEmpty
                        ? '无记录'
                        : todayMedicals.length == 1
                            ? todayMedicals.first.hospitalName
                            : '${todayMedicals.length}个医疗机构',
              ...
              children: [
                Expanded(
                  child: _StatCard(
                    ...
                    subText: pendingMemos.isEmpty
                        ? '全部完成'
                        : pendingMemos.length == 1
                            ? (pendingMemos.first.title ?? '备忘')
                            : '${pendingMemos.length}项待办',
                ...
                children: [
                  Row(
                    ...
                    children: [
                      _WeekStatItem(
                        label: '总打卡',
                        value: '${weekStats['totalCount']}',
                        color: AppColors.success,
                      ),
                      _WeekStatItem(
                        label: '有记录天数',
                        value: '${weekStats['uniqueDays']}',
                        color: AppColors.primary,
                      ),
                      Container(
                        width: 1,
                    ...
                    if (weekStats['uniqueDays'] != null &&
                        (weekStats['uniqueDays'] as int) > 0) ...[
                      SizedBox(height: 14),
                      ClipRRect(
                        borderRadius: BorderRadius.circular(8),
                        child: LinearProgressIndicator(
                          value:
                              (weekStats['uniqueDays'] as int) /
                              now.weekday,
                          minHeight: 8,
                      ...
                      SizedBox(height: 4),
                      Text(
                        '完成率 ${((weekStats['uniqueDays'] as int) / now.weekday * 100).toStringAsFixed(0)}%',
                        style: TextStyle(
                          fontSize: 11,
                          color: AppColors.success,
...
class _ScheduleItem extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    decoration: BoxDecoration(
      ...
      border: Border(
        left: BorderSide(
          color: isChecked ? AppColors.success : schedule.color,
          width: 4,
        ),
      ),
      boxShadow: [
        BoxShadow(color: schedule.color.withValues(alpha: 0.05), blurRadius: 6),
      ],
    ),
    child: Row(
      children: [
        Container(
          ...
          child: Center(
            child: Text(schedule.typeIcon, style: TextStyle(fontSize: 18)),
          ),
        ),
        SizedBox(width: 12),
        Expanded(
...
class _MedicalItem extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    decoration: BoxDecoration(
      ...
      border: Border(
        left: BorderSide(
          color: AppColors.scheduleMedical.withValues(alpha: 0.5),
          width: 3,
        ),
      ),
    ),
        ...
        Expanded(
          child: Column(
            ...
            children: [
              ...
              Text(
                '${record.hospitalName} · ${DateFormat.yMMMd('zh_CN').format(record.visitDate)}',
                style: TextStyle(fontSize: 11, color: AppColors.textSecondary),
              ),
            ],
          ),
